...
from vbot.constants import BAD_WORD_PATTERN, BOT_DESCRIPTIONS, EmbedColor
...
class Events(commands.Cog, name="Events"):
    ...
    async def on_guild_update(self, before: discord.Guild, after: discord.Guild) -> None:
        ...
        logger.info(f"EVENT: Guild {before.name} updated to {after.name}")
        # if before.name != after.name:
        #     logger.info(f"DATABASE: Rename guild `{before.name}` => `{after.name}`") # noqa: ERA001

        #     await DBGuild.find_one(DBGuild.id == before.id).upsert(
        #         Set({"date_modified": time_now, "name": after.name}), # noqa: ERA001
    ...
    async def on_member_join(self, member: discord.Member) -> None:
        ...
        # logger.debug(f"DATABASE: Update user `{member.name}`") # noqa: ERA001
        # await User.find_one(User.id == member.id).upsert(
        #     Set(
        #         {"date_modified": time_now(),
        #             "name": member.display_name, # noqa: ERA001
        #         },
        #     ),
    ...
    async def on_guild_join(self, guild: discord.Guild) -> None:
        ...
        # logger.debug(f"DATABASE: Update guild `{guild.name}`") # noqa: ERA001
        # await DBGuild.find_one(DBGuild.id == guild.id).upsert(
        #     Set({"date_modified": time_now, "name": guild.name}), # noqa: ERA001
